...
def snr_to_noise_std(snr_db, gt_alg=1, coded_pwr=1.0, feat_channels=512):
    ...
    ``(3*224*224) / (feat_channels*28*28)`` for GTGT-FM (gt_alg=2).
    ...
    Args:
        ...
        feat_channels: Channel count of the transmitted (layer2) feature map, i.e. the
                       backbone's ``layer3[0].conv1.in_channels`` — 128 for ResNet-18/34
                       (BasicBlock), 512 for Bottleneck archs (ResNet-50+, ResNeXt,
                       Wide-ResNet).  Sets the GTGT-FM code rate.  Defaults to 512 for
                       backward compatibility (the original hardcoded ResNeXt-101 value).
    ...
    if snr_db is None:
        return None
    if gt_alg == 2:
        code_rate = (3 * 224 * 224) / (feat_channels * 28 * 28)
    else:
        code_rate = 1.0
