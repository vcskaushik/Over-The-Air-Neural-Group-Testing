...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        back_bone_model = models.__dict__[args.arch](pretrained=True,gt=True)
    else:
        print("=> creating model '{}'".format(args.arch))
        back_bone_model = models.__dict__[args.arch](gt=True)
...
def train(train_dataset_list, model, criterion, optimizer, epoch, args):
    ...
    if args.SNR==None:
        ...
        noise_std = np.sqrt(1/(code_rate*(10**(args.SNR/10))))
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.GT_alg == 1:
            ...
            output = model(images,noise_std,args.gpu) 
            ...
            if noise_std == None:
                ...
                output = model(images/args.background_K)
            else:
                images = torch.sum(images, dim=1, keepdim=True)
                noise = torch.normal(mean=0.0,std=noise_std,size = images.size()).detach().cuda(args.gpu, non_blocking=True) 
                output = model((images+noise)/args.background_K)
