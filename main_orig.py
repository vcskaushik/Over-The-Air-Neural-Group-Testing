...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        back_bone_model = models.__dict__[args.arch](args,pretrained=True)
    else:
...
def validate(val_loader, model, criterion, args, dumpResult=False):
    batch_time = AverageMeter('Time', ':6.3f')
    losses_bce = AverageMeter('Loss_bce', ':.4e')
    losses_ce = AverageMeter('Loss_ce', ':.4e')
    top1_gt = AverageMeter('GT Acc@1', ':6.2f')
    top1_std = AverageMeter('Std Acc@1', ':6.2f')
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, losses_bce, losses_ce, top1_gt,top1_std],
        prefix='Test: ')
    ...
    with torch.no_grad():
        ...
        for i, (images, target,target_classes) in enumerate(val_loader):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True)
            target = target.cuda(args.gpu, non_blocking=True)
            ...
            loss_bce  = criterion(output1, target)
            target_classes = target_classes.reshape((-1)).cuda(args.gpu, non_blocking=True).to(torch.int64)
#             print(output1.dtype,output2.dtype)
#             print(target.dtype,target_classes.dtype)
            loss_ce = criterion(output2, target_classes)
            


            # measure accuracy and record loss
            # acc1, acc5 = accuracy(output, target, topk=(1,5))
            acc1_gt = accuracy(output1, target, topk=(1, ))[0]
            acc1_std = accuracy(output2, target_classes, topk=(1, ))[0]
            
            losses_bce.update(loss_bce.item(), images.size(0))
            losses_bce.update(loss_ce.item(), images.size(0))
            top1_gt.update(acc1_gt[0], images.size(0))
            top1_std.update(acc1_std[0], images.size(0))
            ...
            # For analysis
            ##################################
            output_scores = torch.nn.functional.softmax(output1, dim=-1)
        ...
        print('VAL * Acc@1 GT {top1.avg:.3f}'
              .format(top1=top1_gt))
        print('VAL * Acc@1 Std {top1.avg:.3f}'
              .format(top1=top1_std))
        ...
        logging.info('VAL * Acc@1 GT {top1.avg:.3f}'
            .format(top1=top1_gt))
        logging.info('VAL * Acc@1 Std {top1.avg:.3f}'
            .format(top1=top1_std))
...
class GroupTestDataset_val(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_list, args, split, valK=None):
        ...
        with open('data/label_map2.json') as f :
        ...
        ################################## 
        positive_data_list = [ [s[0], positive_target,labels_dict[s[0].split('/')[-2]] ] for s in positive_data_list if s[0].split('/')[-1] in Constants.firearm_file_paths]
        ...
        ################################## 
        negative_target = 0 
        negative_data_list = [ [s[0], negative_target,labels_dict[s[0].split('/')[-2]] ] for s in negative_data_list]
...
#         print(shuffled_mixing_data_list[0])
#         print(shuffled_mixing_data_list.shape)
        ...
        self.dataset_samples = np.array(shuffled_mixing_data_list).reshape( 
            ...
            self.background_K + 1, 
            shuffled_mixing_data_list.shape[-1]
        ).transpose((1,0,2))
    ...
    def __getitem__(self, index):
        ...
        for folder_idx in range(self.background_K + 1): 
        # for folder_idx in [0]: 
            path, target_this, img_class = self.dataset_samples[folder_idx][index]
            target = target or int(target_this) # Note: need to construct each target for group testing right now 
            sample = self.loader(path)
            sample = self.transform(sample)
            images_for_stack_list.append(sample)
            img_classes.append(int(img_class))

        # superimposed_images = superimposed_images / (self.background_K + 1) 
        superimposed_images = torch.stack(images_for_stack_list)
        img_classes = torch.Tensor(img_classes)
        
        return superimposed_images, target, img_classes
