...
import torchvision.models as models #design 1
import numpy as np
import logging
import pickle 
import pathlib
...
def train(train_dataset_list, model, criterion, optimizer, epoch, args):
    ...
    if args.SNR==None:
        ...
        #model.noisy = True
        noise_std = np.sqrt(1/(10**(args.SNR/10)))
...
def validate(val_loader, model, criterion, args, dumpResult=False):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if args.gpu is not None:
                images = images.squeeze().cuda(args.gpu, non_blocking=True)
            
            K = images.shape[1]
            target = target.cuda(args.gpu, non_blocking=True)
            
             # For Model 4 - Pixel level merge with noise
            if noise_std == None:
                images = torch.squeeze(torch.sum(images, dim=1, keepdim=False))
                output = model(images/K)
            else:
