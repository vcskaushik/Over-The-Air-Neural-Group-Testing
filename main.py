...
def get_args_parser():
    ...
    parser.add_argument('--data', metavar='DIR', default='data/ImageNet-ILSVRC2012', 
    ...
    parser.add_argument('--GT-alg', default=1, type=int, metavar='N',
                        help='Group testing algorithm: 1: ITIT, 2: GTGT-FM, 3: ITGT-FM, 4: GTGT-PM')
    ...
    parser.add_argument('--phase', dest='phase', action='store_true',
                        help='enable random phase shift [-pi, pi] on intermediate feature maps')
    ...
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                        help='momentum')
    parser.add_argument('--snr-schedule', default=1, type=int, metavar='N',
                        help='SNR update schedule: 1(default): epoch, 2: batch')
    parser.add_argument('--snr-type', default=1, type=int, metavar='N',
                        help='type of SNR: 1(default): constant, 2: random')
    parser.add_argument('--SNR', type=float, metavar='SNR',
                        help='SNR in dB')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        back_bone_model = models.__dict__[args.arch](pretrained=True,gt=True, phase=args.phase)
    else:
    ...
    coded_pwr = 1 # initial true coded power factor
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            coded_pwr = checkpoint['coded_pwr']
    ...
    if args.evaluate:
        acc1 = validate(group_test_val_loader, model, criterion, coded_pwr, args, dumpResult=True)
        return
    ...
    #Tensorboard Logs Initialization
    tb_path = os.path.join(args.output_dir, "runs")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        coded_pwr = train(train_dataset_list, model, criterion, optimizer, epoch, coded_pwr, tb_writer, args)
        ...
        ##################################
        acc1, avg_loss = validate(group_test_val_loader, model, criterion, coded_pwr, args, dumpResult=True)
        tb_writer.add_scalar('Loss/val', avg_loss, epoch)
        tb_writer.add_scalar('Accuracy/val', acc1, epoch)
...
def train(train_dataset_list, model, criterion, optimizer, epoch, coded_pwr, writer, args):
    ...
    if args.snr_schedule == 1:
        noise_std = snr_update_function(args, epoch)
        if args.SNR is None:
            print("No SNR is specified, equivalent to no noise added.")
            logging.info("No SNR is specified, equivalent to no noise added.")
        else:
            print("SNR for epoch %d: %.1f", epoch, args.SNR)
            logging.info("SNR for epoch {}: {: .1f}".format(epoch, args.SNR))
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.snr_schedule == 2:
            noise_std = snr_update_function(args, i)
        ...
        if args.GT_alg == 1:
            if noise_std == None:
                output, mean_x, power_x = model(images)
            else:
                noise = torch.normal(mean=0.0,std=noise_std,size = images.size()).detach().cuda(args.gpu, non_blocking=True) 
                output, mean_x, power_x = model(images+noise)

        elif args.GT_alg == 2:
            output, mean_x, power_x = model(images,noise_std,args.gpu)
            ...
            if noise_std == None:
                images = torch.sum(images, dim=1, keepdim=True)
                output, mean_x, power_x = model(images/args.background_K)
            else:
                images = torch.sum(images, dim=1, keepdim=True)
                noise = torch.normal(mean=0.0,std=noise_std,size = images.size()).detach().cuda(args.gpu, non_blocking=True) 
                output, mean_x, power_x = model((images+noise)/args.background_K)
        ...
        mean_x_all.append( mean_x.cpu().numpy() )
        power_x_all.append( power_x.cpu().numpy() )
        ...
        if i % args.print_freq == 0 or i == len(train_loader)-1:
            progress.display(i)
        sys.stdout.flush()
    mean_x_all_avg = np.mean(mean_x_all)
    power_x_all_avg = np.mean(power_x_all)
    
    print("train_mean_x_all {:.3f}".format(mean_x_all_avg) )
    print("train_power_x_all {:.3f}".format(power_x_all_avg) )
    
    logging.info("train_mean_x_all {:.3f}".format(mean_x_all_avg) )
    logging.info("train_power_x_all {:.3f}".format(power_x_all_avg))
    
    writer.add_scalar('Loss/train', losses.avg, epoch)
    writer.add_scalar('Accuracy/train', top1.avg, epoch)

    return power_x_all_avg #return the avg coded power after every epoch
...
def validate(val_loader, model, criterion, train_coded_pwr, args, dumpResult=False):
    batch_time = AverageMeter('Time', ':6.3f')
    ...
    if args.snr_schedule == 1:
        noise_std = snr_update_function(args)
        if args.SNR is None:
            print("No SNR is specified, equivalent to no noise added.")
            logging.info("No SNR is specified, equivalent to no noise added.")
        else:
            print("SNR at val: ", args.SNR)
            logging.info("SNR at val: {: .1f}".format(args.SNR))
    ...
    with torch.no_grad():
        ...
        mean_x_all = np.mean(mean_x_all)
        power_x_all = np.mean(power_x_all)
        ...
        print("Val_mean_x_all {:.3f}".format(mean_x_all) )
        print("Val_power_x_all {:.3f}".format(power_x_all) )
        ...
        # if is_main_process():
        print(np.max(target_all),np.max(pred_score_all),np.sum(np.isnan(target_all)),np.sum(np.isnan(pred_score_all)))
        ...
        logging.info("Val_mean_x_all {:.3f}".format(mean_x_all) )
        logging.info("Val_power_x_all {:.3f}".format(power_x_all) )
...
def snr_update_function(args, step_num = 1):
        
    if args.snr_type == 2: # random SNR
        args.SNR = float(np.random.randint(-15, -7+1)) # random SNR between -15 to -7 dB
    ...
    if args.SNR==None:
        ...
        noise_std = np.sqrt(coded_pwr/(code_rate*(10**(args.SNR/10))))
    return noise_std
...
class GroupTestDataset_val(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_list, args, split, valK=None):
        ...
        # Reshape the list 
        ##################################  
        len_list = (len(shuffled_mixing_data_list)//(self.background_K + 1))*(self.background_K + 1)
        #print(len_list)
        shuffled_mixing_data_list = shuffled_mixing_data_list[0:len_list]
        #print("shuffled_mixing_data_list", np.array(shuffled_mixing_data_list).shape)
